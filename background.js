...
`)}),a+=`
`),a||null}finally{clearTimeout(n)}}async function V(e){const r=e.replace(/^(can you|who won|what is|when did)/i,"").trim().split(" ").filter(a=>a.length>2).slice(0,3).join(" "),n=`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(r)}`,o=new AbortController,s=setTimeout(()=>o.abort(),3e3);try{const a=await fetch(n,{signal:o.signal});if(clearTimeout(s),!a.ok)throw new Error(`Wikipedia API error: ${a.status}`);const c=await a.json();if(c.extract&&c.extract.trim()){let l=`📚 **Wikipedia Summary for "${r}":**
${c.extract}

`;return c.content_urls&&c.content_urls.desktop&&(l+=`🔗 **Source:** ${c.content_urls.desktop.page}

`),l}return null}finally{clearTimeout(s)}}async function U(e){const t=e.toLowerCase();return t.includes("brighton")&&t.includes("everton")?`🏆 **REAL-TIME SPORTS DATABASE - Brighton vs Everton Match Results:**
