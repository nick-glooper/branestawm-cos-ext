let d=null,y=null,c=null;chrome.runtime.onInstalled.addListener(async e=>{e.reason==="install"?(console.log("Branestawm extension installed"),await chrome.storage.local.set({settings:{authMethod:null,googleToken:null,apiEndpoint:"https://api.cerebras.ai/v1/chat/completions",apiKey:"",model:"llama3.1-8b",systemPrompt:"You are Branestawm, an indispensable AI Chief of Staff designed to provide cognitive support for neurodivergent users. Always break down complex tasks into clear, manageable steps. Provide patient, structured guidance. Use numbered lists and clear headings to organize information. Focus on being helpful, supportive, and understanding of executive function challenges.",showTooltips:!0,syncKey:"",syncId:"",jsonbinApiKey:"",usePrivateBins:!1,autoSync:!1}}),await chrome.storage.local.set({projects:{default:{id:"default",name:"Default Project",description:"Default project for general conversations",conversations:[],artifacts:[],createdAt:new Date().toISOString()}},conversations:{},artifacts:{},currentProject:"default"}),await S(),await A()):e.reason==="update"&&(console.log("Branestawm extension updated to version:",chrome.runtime.getManifest().version),await U(),await S()),P()});chrome.action&&chrome.action.onClicked.addListener(async e=>{await A()});async function A(){try{if(d)try{const t=await chrome.tabs.get(d);await chrome.tabs.update(d,{active:!0}),await chrome.windows.update(t.windowId,{focused:!0});return}catch{d=null}d=(await chrome.tabs.create({url:chrome.runtime.getURL("index.html"),active:!0})).id,E()}catch(e){console.error("Error opening Branestawm tab:",e)}}chrome.tabs.onRemoved.addListener(async(e,t)=>{if(e===d){d=null,v();const{settings:r}=await chrome.storage.local.get(["settings"]);if(r&&r.autoSync&&r.syncId){console.log("Branestawm tab closed, triggering auto-sync...");try{await T()}catch(n){console.error("Auto-sync failed:",n)}}}});async function T(){try{const e=await chrome.storage.local.get(["settings","projects","conversations","artifacts"]),{settings:t}=e;if(!t.syncId||!t.syncKey){console.log("Auto-sync skipped: missing sync credentials");return}const r={projects:e.projects||{},conversations:e.conversations||{},artifacts:e.artifacts||{},settings:{...t,apiKey:"",googleToken:null,jsonbinApiKey:""},timestamp:new Date().toISOString()};let n=r;t.syncKey&&(n=await _(JSON.stringify(r),t.syncKey));const o=t.usePrivateBins?`https://api.jsonbin.io/v3/b/${t.syncId}`:`https://api.jsonbin.io/v3/b/${t.syncId}`,s={"Content-Type":"application/json"};t.usePrivateBins&&t.jsonbinApiKey&&(s["X-Master-Key"]=t.jsonbinApiKey);const a=await fetch(o,{method:"PUT",headers:s,body:JSON.stringify({data:n,encrypted:!!t.syncKey,syncType:t.usePrivateBins?"private":"public",appVersion:chrome.runtime.getManifest().version,autoSynced:!0})});a.ok?(console.log("Auto-sync completed successfully"),await chrome.storage.local.set({settings:{...t,lastAutoSync:new Date().toISOString()}})):console.error("Auto-sync failed:",a.status,a.statusText)}catch(e){console.error("Auto-sync error:",e)}}class I{constructor(){this.db=null,this.dbName="BranestawmVectorDB",this.version=1,this.ready=!1}async initialize(){return console.log("🔍 VECTOR DB: Initializing IndexedDB vector database..."),new Promise((t,r)=>{const n=indexedDB.open(this.dbName,this.version);n.onerror=()=>{console.error("🔍 VECTOR DB: Failed to open database:",n.error),r(n.error)},n.onsuccess=()=>{this.db=n.result,this.ready=!0,console.log("🔍 VECTOR DB: Database initialized successfully"),t(this.db)},n.onupgradeneeded=o=>{console.log("🔍 VECTOR DB: Setting up database schema...");const s=o.target.result;if(!s.objectStoreNames.contains("documents")){const a=s.createObjectStore("documents",{keyPath:"id"});a.createIndex("type","type",{unique:!1}),a.createIndex("createdAt","createdAt",{unique:!1}),a.createIndex("source","source",{unique:!1})}if(!s.objectStoreNames.contains("embeddings")){const a=s.createObjectStore("embeddings",{keyPath:"id"});a.createIndex("docId","docId",{unique:!1}),a.createIndex("chunkIndex","chunkIndex",{unique:!1}),a.createIndex("embeddingType","embeddingType",{unique:!1})}s.objectStoreNames.contains("metadata")||s.createObjectStore("metadata",{keyPath:"key"})}})}async storeDocument(t,r,n={}){if(!this.ready)throw new Error("Vector database not initialized");console.log("🔍 VECTOR DB: Storing document:",t);const s=this.db.transaction(["documents"],"readwrite").objectStore("documents"),a={id:t,content:r,type:n.type||"unknown",source:n.source||"user",title:n.title||t,chunks:this.chunkText(r),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),metadata:n};return new Promise((i,l)=>{const u=s.put(a);u.onsuccess=()=>{console.log("🔍 VECTOR DB: Document stored successfully"),i(a)},u.onerror=()=>{console.error("🔍 VECTOR DB: Failed to store document:",u.error),l(u.error)}})}async storeEmbedding(t,r,n,o="simple"){if(!this.ready)throw new Error("Vector database not initialized");const a=this.db.transaction(["embeddings"],"readwrite").objectStore("embeddings"),i={id:`${t}-${r}`,docId:t,chunkIndex:r,embedding:Array.from(n),embeddingType:o,createdAt:new Date().toISOString()};return new Promise((l,u)=>{const m=a.put(i);m.onsuccess=()=>l(i),m.onerror=()=>u(m.error)})}async searchSimilar(t,r=5,n=.5){if(!this.ready)throw new Error("Vector database not initialized");console.log("🔍 VECTOR DB: Searching for similar embeddings...");const o=this.db.transaction(["embeddings","documents"],"readonly"),s=o.objectStore("embeddings"),a=o.objectStore("documents");return new Promise((i,l)=>{const u=[],m=s.openCursor();m.onsuccess=async D=>{const w=D.target.result;if(w){const h=w.value,g=this.cosineSimilarity(t,h.embedding);g>=n&&u.push({docId:h.docId,chunkIndex:h.chunkIndex,similarity:g,embeddingType:h.embeddingType}),w.continue()}else{u.sort((f,b)=>b.similarity-f.similarity);const h=u.slice(0,r),g=[];for(const f of h){const b=a.get(f.docId),p=await new Promise(C=>{b.onsuccess=()=>C(b.result)});p&&p.chunks[f.chunkIndex]&&g.push({...f,content:p.chunks[f.chunkIndex],title:p.title,type:p.type,source:p.source})}console.log(`🔍 VECTOR DB: Found ${g.length} similar chunks`),i(g)}},m.onerror=()=>l(m.error)})}async getStatistics(){if(!this.ready)throw new Error("Vector database not initialized");const t=this.db.transaction(["documents","embeddings"],"readonly"),r=t.objectStore("documents"),n=t.objectStore("embeddings"),o=await new Promise((a,i)=>{const l=r.count();l.onsuccess=()=>a(l.result),l.onerror=()=>i(l.error)}),s=await new Promise((a,i)=>{const l=n.count();l.onsuccess=()=>a(l.result),l.onerror=()=>i(l.error)});return{documentCount:o,embeddingCount:s,ready:this.ready}}chunkText(t,r=500,n=50){const o=[],s=t.split(/[.!?]+/).filter(i=>i.trim().length>0);let a="";for(const i of s)a.length+i.length>r&&a.length>0?(o.push(a.trim()),a=a.split(" ").slice(-n).join(" ")+" "+i):a+=(a?" ":"")+i;return a.trim()&&o.push(a.trim()),o.length>0?o:[t]}createSimpleEmbedding(t){const r=t.toLowerCase().split(/\s+/).filter(a=>a.length>2),n={};r.forEach(a=>{n[a]=(n[a]||0)+1});const o=new Array(128).fill(0);Object.keys(n).forEach(a=>{const i=this.simpleHash(a)%128;o[i]+=n[a]});const s=Math.sqrt(o.reduce((a,i)=>a+i*i,0));if(s>0)for(let a=0;a<o.length;a++)o[a]/=s;return o}simpleHash(t){let r=0;for(let n=0;n<t.length;n++){const o=t.charCodeAt(n);r=(r<<5)-r+o,r=r&r}return Math.abs(r)}cosineSimilarity(t,r){if(t.length!==r.length)return 0;let n=0,o=0,s=0;for(let a=0;a<t.length;a++)n+=t[a]*r[a],o+=t[a]*t[a],s+=r[a]*r[a];return o===0||s===0?0:n/(Math.sqrt(o)*Math.sqrt(s))}}async function S(){console.log("🧠 Background: Initializing vector database...");try{c||(c=new I),await c.initialize(),console.log("🧠 Background: Vector database initialized successfully"),await B()}catch(e){console.error("🧠 Background: Failed to initialize vector database:",e)}}async function B(){if(!(!c||!c.ready)){console.log("🧠 Background: Processing existing data for embedding...");try{const e=await chrome.storage.local.get(["conversations","artifacts","projects"]);if(e.conversations)for(const[t,r]of Object.entries(e.conversations))await R(t,r);if(e.artifacts)for(const[t,r]of Object.entries(e.artifacts))await L(t,r);if(e.projects)for(const[t,r]of Object.entries(e.projects))await x(t,r);console.log("🧠 Background: Existing data processing complete")}catch(e){console.error("🧠 Background: Error processing existing data:",e)}}}async function R(e,t){var r;if(!(!c||!c.ready))try{const n=((r=t.messages)==null?void 0:r.map(o=>`${o.role}: ${o.content}`).join(`
`))||"";if(n.trim()){await c.storeDocument(`conv-${e}`,n,{type:"conversation",source:"branestawm",title:t.title||`Conversation ${e}`,projectId:t.projectId,createdAt:t.createdAt});const o=c.chunkText(n);for(let s=0;s<o.length;s++){const a=c.createSimpleEmbedding(o[s]);await c.storeEmbedding(`conv-${e}`,s,a,"simple")}}}catch(n){console.error(`🧠 Background: Error embedding conversation ${e}:`,n)}}async function L(e,t){if(!(!c||!c.ready))try{const r=`${t.title||""}
${t.content||""}`.trim();if(r){await c.storeDocument(`artifact-${e}`,r,{type:"artifact",source:"branestawm",title:t.title||`Artifact ${e}`,artifactType:t.type,projectId:t.projectId});const n=c.chunkText(r);for(let o=0;o<n.length;o++){const s=c.createSimpleEmbedding(n[o]);await c.storeEmbedding(`artifact-${e}`,o,s,"simple")}}}catch(r){console.error(`🧠 Background: Error embedding artifact ${e}:`,r)}}async function x(e,t){if(!(!c||!c.ready))try{const r=`${t.name||""}
${t.description||""}`.trim();if(r){await c.storeDocument(`project-${e}`,r,{type:"project",source:"branestawm",title:t.name||`Project ${e}`,createdAt:t.createdAt});const n=c.chunkText(r);for(let o=0;o<n.length;o++){const s=c.createSimpleEmbedding(n[o]);await c.storeEmbedding(`project-${e}`,o,s,"simple")}}}catch(r){console.error(`🧠 Background: Error embedding project ${e}:`,r)}}async function O(e,t,r={}){if(!c||!c.ready){console.log("🧠 Background: Vector database not ready, queuing for later");return}try{await c.storeDocument(e,t,r);const n=c.chunkText(t);for(let o=0;o<n.length;o++){const s=c.createSimpleEmbedding(n[o]);await c.storeEmbedding(e,o,s,"simple")}console.log(`🧠 Background: Added document ${e} to vector database`)}catch(n){console.error(`🧠 Background: Error adding ${e} to vector database:`,n)}}async function M(e,t={}){if(!c||!c.ready)return console.log("🧠 Background: Vector database not ready"),[];try{const r=c.createSimpleEmbedding(e),n=await c.searchSimilar(r,t.topK||5,t.threshold||.1);return console.log(`🧠 Background: Vector search returned ${n.length} results`),n}catch(r){return console.error("🧠 Background: Error searching vector database:",r),[]}}function P(){if(!chrome.contextMenus){console.log("Context menus API not available");return}try{chrome.contextMenus.create({id:"branestawm-help",title:"Ask Branestawm about this",contexts:["selection"]}),chrome.contextMenus.create({id:"branestawm-plan",title:"Help me plan this task",contexts:["selection"]}),chrome.contextMenus.create({id:"branestawm-break-down",title:"Break this down into steps",contexts:["selection"]})}catch(e){console.error("Error setting up context menus:",e)}}chrome.contextMenus&&chrome.contextMenus.onClicked.addListener(async(e,t)=>{const r=e.selectionText;let n="";switch(e.menuItemId){case"branestawm-help":n=`Help me understand this: "${r}"`;break;case"branestawm-plan":n=`Help me create a plan for this task: "${r}"`;break;case"branestawm-break-down":n=`Break this down into clear, manageable steps: "${r}"`;break}n&&(await A(),await chrome.storage.local.set({pendingQuery:n,pendingQueryTimestamp:Date.now()}))});function E(){y||(y=setInterval(()=>{chrome.runtime.getPlatformInfo().then(()=>{console.log("Service worker keepalive ping")})},25e3))}function v(){y&&(clearInterval(y),y=null)}chrome.runtime.onConnect.addListener(e=>{e.name==="branestawm-keepalive"&&(E(),e.onDisconnect.addListener(()=>{v()}))});chrome.runtime.onMessage.addListener((e,t,r)=>{var n;if(console.log("📨 Background: Received message:",e.type,"from:",((n=t.tab)==null?void 0:n.url)||"unknown"),e.type==="SYNC_REQUEST")return(async()=>{try{await T(),r({success:!0})}catch(o){r({success:!1,error:o.message})}})(),!0;if(e.type==="GET_TAB_ID"&&r({tabId:d}),e.type==="IMPORT_SEARCH_RESULTS"){console.log("📥 Background: Received search results import from:",e.source);const o=`import_${Date.now()}_${Math.random().toString(36).substr(2,9)}`;return r({success:!0,importId:o,status:"processing"}),chrome.storage.local.set({[`searchImport_${o}`]:{source:e.source,query:e.query,content:e.content,url:e.url,timestamp:e.timestamp,status:"ready"}}).then(()=>{console.log("✅ Background: Import data stored with ID:",o)}).catch(s=>{console.error("❌ Background: Failed to store import data:",s),chrome.storage.local.set({[`searchImport_${o}`]:{source:e.source,query:e.query,content:e.content,url:e.url,timestamp:e.timestamp,status:"error",error:s.message}})}),!1}if(e.type==="WEB_SEARCH"){console.log("🔍 Background: Performing web search for:",e.query);const o=e.searchId||`search_${Date.now()}`;return r({success:!0,searchId:o,status:"started"}),j(e.query).then(async s=>{console.log("✅ Background: Web search completed, storing results"),await chrome.storage.local.set({[`webSearch_${o}`]:{status:"completed",results:s,timestamp:Date.now()}})}).catch(async s=>{console.error("❌ Background: Web search failed:",s),await chrome.storage.local.set({[`webSearch_${o}`]:{status:"error",error:s.message,timestamp:Date.now()}})}),!1}if(e.type==="INIT_LOCAL_AI")return console.log("🧠 Background: Initializing Local AI (EmbeddingGemma)"),(async()=>{try{await $(),console.log("🧠 Offscreen document created - model will auto-initialize"),console.log("🧠 Responding to options page"),r({success:!0})}catch(o){console.error("❌ Background: Failed to initialize Local AI:",o),r({success:!1,error:o.message})}})(),!0;if(e.type==="CHECK_LOCAL_AI_STATUS")return(async()=>{try{const o=await k();r(o?{ready:!1,loading:!0,hasModel:!1}:{ready:!1,loading:!1,hasModel:!1})}catch(o){console.error("Error checking Local AI status:",o),r({ready:!1,loading:!1,hasModel:!1,error:o.message})}})(),!0;if(e.type==="LOCAL_AI_STATUS")return console.log("🧠 Local AI Status:",e.status,e.progress?`(${e.progress}%)`:""),!1;if(e.type==="LOCAL_AI_ERROR")return console.error("🧠 Local AI Error:",e.error),!1;if(e.type==="OFFSCREEN_READY")return console.log("🧠 Offscreen document is ready"),!1;if(e.type==="ADD_TO_VECTOR_DB")return(async()=>{try{await O(e.id,e.content,e.metadata||{}),r({success:!0})}catch(o){console.error("🧠 Background: Error adding to vector database:",o),r({success:!1,error:o.message})}})(),!0;if(e.type==="SEARCH_VECTOR_DB")return(async()=>{try{const o=await M(e.query,e.options||{});r({success:!0,results:o})}catch(o){console.error("🧠 Background: Error searching vector database:",o),r({success:!1,error:o.message,results:[]})}})(),!0;if(e.type==="GET_VECTOR_DB_STATS")return(async()=>{try{if(c&&c.ready){const o=await c.getStatistics();r({success:!0,stats:o})}else r({success:!0,stats:{documentCount:0,embeddingCount:0,ready:!1}})}catch(o){console.error("🧠 Background: Error getting vector database stats:",o),r({success:!1,error:o.message,stats:null})}})(),!0});async function $(){if(await k()){console.log("🧠 Offscreen document already exists");return}console.log("🧠 Creating offscreen document for Local AI"),await chrome.offscreen.createDocument({url:"offscreen.html",reasons:["DOM_SCRAPING"],justification:"Local AI processing with EmbeddingGemma requires WebGPU access and transformers.js execution"}),console.log("✅ Offscreen document created successfully")}async function k(){try{return(await chrome.runtime.getContexts({contextTypes:["OFFSCREEN_DOCUMENT"]})).length>0}catch{return!1}}async function j(e){console.log(`🔍 Background: Starting web search for: "${e}"`);const t=[()=>N(e),()=>W(e),()=>V(e)];for(let r=0;r<t.length;r++)try{const n=await t[r]();if(n)return console.log(`✅ Background: Web search successful using method ${r+1}`),n}catch(n){console.warn(`⚠️ Background: Search method ${r+1} failed:`,n.message)}return console.error("❌ Background: All web search methods failed"),"🌐 Web search attempted but all methods failed. The search functionality is experiencing technical difficulties."}async function N(e){const t=`https://api.duckduckgo.com/?q=${encodeURIComponent(e)}&format=json&no_redirect=1&no_html=1&skip_disambig=1`,r=new AbortController,n=setTimeout(()=>r.abort(),5e3);try{const o=await fetch(t,{signal:r.signal,headers:{"User-Agent":"Mozilla/5.0 (compatible; BranestawmBot/1.0)"}});if(clearTimeout(n),!o.ok)throw new Error(`DuckDuckGo API error: ${o.status}`);const s=await o.json();let a="";return s.Abstract&&s.Abstract.trim()&&(a+=`📝 **Summary:** ${s.Abstract}
